...
void SMime::setPublicKey(const char *filename)
{
    _publicKeyfile = QString::fromLatin1(filename);
    loadPKCS12PublicKey();
}
...
{
    MimePart& mimeMultiPart = _mimeMessage->getContent();
    QByteArray contentBoundary = ((SimpleMail::MimeMultiPart*) &_mimeMessage->getContent())->getBoundary();

    if (typeid(mimeMultiPart) == typeid(MimeMultiPart)) {
        QList<MimePart *> parts = static_cast<MimeMultiPart*>(&mimeMultiPart)->parts();
        static_cast<MimeMultiPart*>(&mimeMultiPart)->removeParts();

        MimeMultiPart *multiPartSigned = new MimeMultiPart(MimeMultiPart::Signed);
        MimeMultiPart *multiPartCopy = new MimeMultiPart(((SimpleMail::MimeMultiPart*) &_mimeMessage->getContent())->mimeType());
        multiPartCopy->setBoundary(contentBoundary);

        for(int i = 0; i < parts.size(); i++)
        {
            multiPartCopy->addPart(parts.at(i));
        }

        multiPartSigned->addPart(multiPartCopy);
        _mimeMessage->setContent(multiPartSigned);
    }
}
